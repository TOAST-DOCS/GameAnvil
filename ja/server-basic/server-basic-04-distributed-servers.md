...
では、このようなIPCのためにプロセスはどのように相互接続するのでしょうか？その答えはMeetpointです。GameAnvilはMeetpointアドレスを設定できます。次のような形で、1つ以上のIPアドレスのペアを設定できます。GameAnvilプロセスは、初回駆動時に設定されたMeetpointアドレスのいずれかに接続を試みて、サーバー群全体の情報を同期します。
