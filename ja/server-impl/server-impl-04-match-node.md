...
| onRefill | マッチリフィルリクエストを処理 | ユーザー/パーティマッチメイキングの過程で任意のユーザーが退出した場合、新しいユーザーを補充するための処理を行います。一般的に、マッチメイキングされたルームに対してonLeaveRoomが呼び出されるときにmatchRefillを呼び出して連動させることができます。つまり、マッチングされたルームから誰かが退出したときにリフィルをリクエストするものです。リフィルはキューに積まれているマッチリクエストは使用しません。リフィルリクエスト以降に来る新しいマッチリクエストのみが対象となります。 |
