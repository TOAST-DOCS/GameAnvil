...
## Game > GameAnvil > サーバー開発ガイド > メッセージハンドリング { #game-gameanvil-server-development-guide-message-handling }
...
## 一般メッセージ処理 { #handle-general-messages }
...
### メッセージハンドラ実装及びメッセージとハンドラの接続 { #implement-message-handler-and-connect-messages-to-handlers }
...
## httpリクエストの処理 { #handle-http-request }
...
### httpパケットディスパッチャの作成及びメッセージとハンドラの接続 { #create-http-packet-dispatcher-and-connect-messages-to-handlers }
