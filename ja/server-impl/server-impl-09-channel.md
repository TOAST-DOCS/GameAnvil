...
"game": [
  {
    "serviceId": 1, // サーバー群でユニークである必要があります [0 ～ 99]
    "serviceName": "MyGame", // サーバー群でユニークである必要があります
    "nodeCnt": 4, // ノード数。nodeCntが0の場合、Game Nodeは作成されません。マシンのコア数を入力することを推奨 [0 ～99]
    "channelIDs": [
      ["初心者"],
      ["初心者"],
      ["上級者"],
...
    ], // ノードごとに付与するチャネルID。(ユニークでなくても可。""という文字列でチャネルを区別せずに重複使用も可能)
    "userTimeout": 5000       // disconnect以降のユーザーオブジェクト除去タイムアウト。 [0 ～ 604800000(7日)]
  }
],
```
