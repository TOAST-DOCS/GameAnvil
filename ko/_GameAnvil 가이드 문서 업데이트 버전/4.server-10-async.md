...
하지만 이러한 쿼리를 기본적으로 비동기 처리해주는 [MySQL X DevAPI](https://dev.mysql.com/doc/x-devapi-userguide/en/)나 [jasync-sql](https://github.com/jasync-sql/jasync-sql)과 같은 비동기 드라이버가 있습니다. GameAnvil은 이 두가지 모두를 지원합니다. 하지만 MySQL X DevAPI는 몇 가지 결함이 발견되어 베타 버전의 독립된 라이브러리 형태로만 제공합니다. 즉, GameAnvil은 jasync-sql을 기본 비동기 
...
JAsyncSql jasync = new JAsyncSql(new com.github.jasync.sql.db.Configuration(
            "gameanvil",
            "10.160.194.129",
            13306,
            "%gameanvil1",
            "GameDB_1"), 63));  // 노드 총 16개 기준으로 노드당 63개 설정
