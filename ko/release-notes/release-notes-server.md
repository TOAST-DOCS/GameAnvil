...
## Game > GameAnvil > 릴리스 노트 > GameAnvil { #game-gameanvil-release-notes-gameanvil }
...
```java
var builder = ProtoSerializer.ObjectListCodec.writer();
builder.addInt32(getRoomId()); // 0
builder.addInt32(roomMode.getId()); // 1
return builder.buildByteString();
```
```java
var reader = ProtoSerializer.ObjectListCodec.reader(byteString);
var roomId = reader.getInt32(0);
var roomMode = RoomMode.getById(reader.getInt32(1));
```
