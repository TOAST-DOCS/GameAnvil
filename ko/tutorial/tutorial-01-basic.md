...
IntelliJ 오른쪽 상단의 버튼 그룹에서 **New Project**를 클릭한 뒤 왼쪽 목록을 스크롤하여 하단의 **Templates**에 있는 **GameAnvil 2.2.0 Template**을 선택합니다. 프로젝트 이름을 설정합니다. 이름에 공백이 있어서는 안 됩니다. 프로젝트 위치와 베이스 패키지 이름을 확인한 뒤 프로젝트를 생성합니다.
...
게임 노드는 필요로 하는 양에 따라, 또는 서버 성능에 따라서 여러 개의 VM으로 구성하여 실행할 수 있습니다. 게임 노드를 몇 개를 실행시킬 것인지에 대한 설정을 하면 서버 실행 시에 자동으로 설정 파일을 읽어서 정해진 개수의 노드를 띄우도록 되어 있습니다. 템플릿 설정에는 게임 노드를 띄우지 않도록 세팅되어 있으므로, 1개를 띄우도록 수정합니다. 추가 수정할 부분은 game 부분의 serviceName과 channelIDs입니다.
...
프로젝트 패널에서 Main 클래스가 위치한 경로를 마우스 오른쪽 버튼으로 클릭한 뒤 **New > Package**를 선택해 **user**라는 이름의 새로운 패키지를 생성합니다. 그리고 **user** 패키지를 다시 마우스 오른쪽 버튼으로 클릭한 뒤 **New > GameAnvil GameUser**를 선택합니다. 파일 생성 대화 상자가 열리면 **File name**에 **SyncGameUser**를, **Service name**에 **Sync**를, **User type**에 **USER_TYPE_SYNC**를 입력한 뒤 **OK**를 클릭합니다.
...
프로젝트 패널에서 Main 클래스가 위치한 경로를 마우스 오른쪽 버튼으로 클릭한 뒤 **New > Package**를 선택해 **room**라는 이름의 새로운 패키지를 생성합니다. 그리고 **room** 패키지를 다시 마우스 오른쪽 버튼으로 클릭한 뒤 **New > GameAnvil GameRoom**을 선택합니다. 파일 생성 대화 상자가 열리면 **File name**에 **SyncGameRoom**을, **Service name**에 **Sync**를, **Room type**에 **ROOM_TYPE_SYNC**를, **User**에 **SyncGameUser**를 입력한 뒤 **OK**를 클릭합니다.
...
템플릿으로 **Universal 2D**를 선택하고 프로젝트 이름과 저장 위치를 확인한 뒤 **Create project**를 클릭합니다. 
이 예제에서 사용한 Unity 버전은 6000.0.59f2이며, 실습 시 다른 버전을 사용해도 무방하나 이 예제의 설명과 차이가 있을 수 있습니다. 
...
**File > Build Profiles**에서 **Scene List**에 IntroScene을 추가하여 빌드 시 포함되도록 설정합니다.
