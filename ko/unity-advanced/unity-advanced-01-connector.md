...
[Unity 기초 개발 가이드 > GameAnvilConnector](../unity-basic/unity-basic-02-gameanvil-manager) 문서에서 커넥터의 주요 기능을 간편하게 사용할 수 있는 GameAnvilConnector에 대해서 다루었습니다.

이번 문서에서는 커넥터를 더욱 확장성 있게 사용할 수 있는 Connector에 대하여 설명합니다.
...
`Connector.Config`를 미리 생성해 놓고 이를 매개변수로 받아서 Connector를 생성할 수도 있습니다. 또 이렇게 생성한`Connector.Config`를 MonoBehavior의 멤버로 만들어 놓을 경우 Unity의 Inspector 창에서 설정값을 바꿀 수도 있습니다.
...
| packetTimeout               | 패킷이 지정된 시간 안에 업데이트되지 않으면 disconnect되었다고 판단. pingInterval보다 큰 값으로 설정. | 5(sec) |
...
Connector.EnablePingPongLogs()로 PingPong에 의해 발생하는 로그 출력 여부를 설정할 수 있습니다. PingPong은 서버와 연결이 잘 되어 있는지 확인하기 위한 패킷을 주고받는 과정입니다.
...
| 로그 레벨 | 설명 |
| :------: | --- |
| Debug | 서버와 클라이언트가 주고받는 모든 패킷의 상세 로그(개발용)|
| Info | 서버와 클라이언트가 주고받는 모든 패킷의 간략 로그 |
...
| Error | 소켓 오류, 서버로부터 받은 오류, 알 수 없는 오류 등의 로그 |
...
서버와 주고받는 메시지 처리를 위해 Update() 함수를 주기적으로 호출해야 합니다. MonoBehaviour의 Update()에서 호출하도록 하는 것이 제일 간편한 방법이지만, 필요에 따라 다른 방식으로 호출해도 무방합니다. 하지만 Thread safe하지 않으므로 별도의 Thread에서 호출하면 안됩니다. 
...
MonoBehaviour를 상속 받아 커넥터를 관리하는 ConnectorHandler 스크립트 예시입니다. 필요에 따라 새로 만들거나 적절하게 변경해 사용하면 됩니다.
...
namespace GameAnvil
{
    public class ConnectorHandler : MonoBehaviour
    {
        ...
        private void OnApplicationPause(bool pause)
        {
            ...
            if (connector.IsConnected())
            {
                // 앱이 resume된 뒤 해야 할 작업이 있다면 여기에서 처리한다.
            }
        }
    }
}
